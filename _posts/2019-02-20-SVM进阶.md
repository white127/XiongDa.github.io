...
将此转换成数学量化的目标:<br>
<center>$$\arg\max_{W,b}margin(W,b)\,st.\ every\ y^{(n)}(W^TX^{(n)}+b)>0$$</center>
其中，根据点到平面的距离公式：<br>
<center>$$d=\frac{|W^TX^{(n)}+b|}{\sqrt{W_1^2+W_2^2+...+W_k^2}}=\frac{y^{(n)}(W^TX^{(n)}+b)}{||W||}$$</center>

因为$y_n$取值为+1或-1，通过引入$y_n$可以把分子的绝对值去掉，这样有<br>
<center>$$margin(W,b)=\min_{n=0...N}(\frac{1}{||W||}y^{(n)}(W^TX^{(n)}+b))$$</center>
...
对于上面的目标函数，可以进一步的简化：<br>
对任意一个待选平面($W^{(i)},b^{(i)}$)，设所有样本点到其平面的最小距离$d=\frac{K}{||W^{(i)}||}$，那么原目标函数等价变换为<br>
<center>$$\arg\max_{W^{(i)},b^{(i)}}{\frac{K}{||W^{(i)}||}},\ st\.\ every\ y^{(n)}({W^{i}}^TX^{(n)}+b^{(i)})$$</center>
...
*<==>*  
<cneter>$$\arg\max_{U^{(i)},c^{(i)}}\frac{1}{||U^{(i)}||}\ st\.\ every\ y^{(n)}({U^{(i)}}^TX^{(n)}+c^{(i)})>=1,\ formula(1.1)$$</center>
