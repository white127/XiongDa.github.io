...
找到对偶问题的最优解$\alpha^\*$：
<center>
 $$\alpha^\*=\arg\max_{\alpha}g(\alpha),\ st.\ to\ \alpha^{(n)}>=0,\sum_{n=1}^N(\alpha^{(n)}y^{(n)})=0$$
</center>

得到最优的$\alpha^\*$之后，利用KKT条件中的第3个条件，可以计算得到$W_i^\*$。利用KKT条件中的第5个条件，当$\alpha^\*=0$时，可以得到$b^\*$，这样就得到了原问题需要求解的最优解$(W^\*,b\*)$。
